...
- v2.13: **🦋 Flyers actually fly** — winged critters now **soar high above the tree canopy** (visual altitude 3 → 12) with **bigger, clearly-visible wings** and a **proper up-and-down wingbeat** (was rotating on the wrong axis — a twist, not a flap). They **cruise right across the biome** in long sweeping arcs (steady cruise speed + edge-bouncing) instead of bobbing in place, **swoop down to graze when hungry and climb back up when full** (yes — they eat; foraging is horizontal, altitude is visual), and start a touch more common (~18%) so there's always a visible flock aloft.
- v2.12: **🧠 Smartness shapes the canned lines** — a critter's brightness (its species smarts + a little hard-won age) now drives *what kind* of thing it says when calm: sharp critters (Foxlings) muse and get existential/meta ("we are matter, contemplating matter", "i'd unionize, but who's the boss"), while simpler ones (Pebbles) keep it concrete and goofy ("grass good", "pretty rock"). Verified live by species: the simple lines came from the dim species, the witty ones from the smartest. (The local-LLM path was already smartness-aware; this brings the offline default in line.)
- v2.11: **🗣 Chatter density + variety fix** — verified live and fixed two problems: it was too sparse (cooldown 2.2–5s → ~0.7–2.1s, no more long pause after each conversation, conversations no longer block others from chiming in), and it was repetitive (a hunt spread panic across the whole herd, so it was a wall of "RUN!"). Speaker selection now **prefers calm critters ~80% of the time**, the hunt only colours a fraction of the herd's lines, and the panic reactions are rarer + anti-repeated — so the personality/meta/idle jokes actually surface (measured: ~30 unique lines in a 30-second window, vs ~13 all-panic before).
- v2.10: **😂 Funnier critter chatter** — a comedy pass: each **species now has its own voice** (Foxling smug, Hopkin anxious, Slink chaotic, Pebble zen), a new vein of **meta-humor** (the critters are dimly aware a "giant" is watching them in a sim), every line pool got bigger and sharper (deadpan/absurd, not just "cute"), and an **anti-repeat** so you stop seeing the same line twice in a row.
- v2.9: **🚀 Ship prep** — MIT license, package metadata, browser-voice default, condensed README.
- v2.8: **🪶 Pre-ship visual polish** — removed the **aurora borealis** (its coloured curtains detracted from a clean night sky), and refined the **terrain**: a touch more resolution, **rock bleeding onto steep faces** (cliffs read as rock, not grass), gentle **valley shading**, and a faint per-vertex grain so the ground no longer looks like a smooth flat-coloured gradient.
